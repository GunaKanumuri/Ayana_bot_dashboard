    ...
    <>
        ...
        <div className="nav-inner">
          ...
          <div className="nav-links">
            ...
            <button className="nav-cta" onClick={openModal}>
              <WhatsAppIcon size={16} /> Start free trial
            </button>
          </div>
        </div>
      </nav>
          ...
          <div className="hero-content">
            <div className="hero-text">
              ...
              <p className="hero-sub">
                AYANA sends a warm voice message to your parents each morning on WhatsApp — in their language.
                They tap a button. You get a daily health report. No app needed.
              </p>
              <div className="hero-actions">
                <button className="btn-primary" onClick={openModal}>
                  <WhatsAppIcon /> Start 14-day free trial
                </button>
              ...
              <div className="hero-proof">
                <div className="proof-faces">
                  {["S", "R", "P", "A"].map((l, i) => (
                    <div key={i} className={`proof-face f${i + 1}`}>{l}</div>
                  ))}
                </div>
                <div className="proof-text">
                  Built for <strong>NRI families</strong> who worry about parents back home
                </div>
              </div>
            </div>
        ...
        <div className="container">
          ...
          <div className="steps-grid">
            {[
              ...
              { icon: "🎙️", iconCls: "amber", title: "Parent hears a warm voice", desc: "Every morning, your parent gets an audio message in their language asking how they are. They tap one button to respond. Zero typing needed.", time: "Happens automatically" },
              { icon: "📊", iconCls: "teal", title: "You get daily peace of mind", desc: "Every evening, you receive a summary: mood, medicines, concerns. If anything is wrong, you know immediately — not days later.", time: "Every day at 8 PM" },
            ].map((s, i) => (
        ...
        <div className="container">
          ...
          <div className="exp-grid">
            {[
              ...
              { icon: "🧠", title: "It remembers and adapts", desc: "If your mom mentioned knee pain yesterday, today it asks \"How is the knee?\" Not \"How are you?\" It tracks health events across days." },
              { icon: "🌙", title: "Good night with your name", desc: "\"Good night Bujjamma! Guna misses you.\" The last thing she hears before sleep is warmth from you — through AYANA." },
            ].map((c, i) => (
        ...
        <div className="container">
          ...
          <div className="price-card reveal">
            ...
            <div className="price-list">
              {[
                "Daily voice check-ins in their language",
                "Medicine reminders (grouped by routine)",
                "Emergency one-tap alert with auto-call",
                "Daily + weekly health reports",
                "Context-aware health tracking",
                "Add siblings free (shared reports)",
        ...
        <div className="container">
          ...
          <div className="faq-list">
            {[
              ...
              { q: "Is my parent's data safe?", a: "All messages go through WhatsApp's end-to-end encryption. We store health check-in data to generate your reports and track patterns. We never sell data, never share with third parties, and you can delete everything anytime." },
              { q: "Can my sibling also receive reports?", a: "Yes, free of charge. Add their WhatsApp number and they'll receive the same daily reports. Any family member can also send custom messages to the parent through AYANA." },
            ].map((faq, i) => (
      ...
      <section className="final-cta-section">
        <div className="container" style={{ textAlign: "center" }}>
          <div className="section-heading reveal" style={{ color: "white", maxWidth: 540, margin: "0 auto" }}>
            Your parent&apos;s first check-in is minutes away
          </div>
          <p className="reveal" style={{ color: "rgba(255,255,255,0.65)", maxWidth: 420, margin: "16px auto 32px", fontSize: 16, lineHeight: 1.7 }}>
            Set up takes 2 minutes. No credit card. Your parent gets their first voice message today.
          </p>
          <button className="btn-primary reveal" onClick={openModal} style={{ fontSize: 18, padding: "18px 44px" }}>
            <WhatsAppIcon size={22} /> Start free trial
          </button>
        </div>
      </section>
    ...
    <div className="phone-frame">
      ...
      <div className="phone-screen">
        <div className="wa-header">
          ...
          <div>
            <div className="wa-name">AYANA Care</div>
        ...
        <div className="chat-area">
          ...
          <div className="bubble user">
            1<div className="time">8:02 AM ✓✓</div>
          </div>
          {/* Medicine question */}
          <div className="bubble bot">
          ...
          {/* User reply */}
          <div className="bubble user">
            1<div className="time">8:03 AM ✓✓</div>
          </div>
          {/* Confirmation */}
          <div className="bubble bot" style={{ fontSize: 12 }}>
            బాగుంది! టిఫిన్ తిని మిగతా మాత్రలు వేసుకో. జాగ్రత్త! 🙏
            <div className="time">8:03 AM</div>
          </div>
        </div>
      </div>
    </div>
...
function OnboardingModal({ onClose }) {
  ...
  const canProceed = step === 1
    ? form.child_name.trim() && form.child_phone.trim()
    : form.parent_nickname.trim() && form.parent_phone.trim();

  const normalizePhone = (raw) => {
    let cleaned = "";
    for (const ch of raw.trim()) {
      if (ch === "+" && !cleaned) cleaned += ch;
...
nav {
  ...
  border-bottom: 1px solid rgba(20,92,72,0.05);
  padding: 14px 0;
...
.logo {
  ...
  color: var(--forest-deep); text-decoration: none; letter-spacing: -0.5px;
}
...
.hero::before {
  ...
  background: radial-gradient(circle at 50% 40%, rgba(20,92,72,0.04) 0%, transparent 70%);
  pointer-events: none;
}
.hero-content {
...
.hero-badge {
  ...
  padding: 7px 18px 7px 12px; border-radius: 50px; font-size: 13px; font-weight: 500;
  margin-bottom: 28px;
  border: 1px solid var(--sage-mid);
  opacity: 0; animation: slideUp 0.7s ease forwards;
}
.pulse-dot {
...
.pulse-dot::after {
  ...
  border-radius: 50%; border: 1.5px solid var(--wa-green);
...
.hero h1 {
  font-family: var(--serif); font-size: clamp(36px, 5.2vw, 58px);
  line-height: 1.12; color: var(--forest-deep); max-width: 560px;
...
.hero-sub {
  ...
  max-width: 460px; margin: 20px 0 32px; font-weight: 400;
  opacity: 0; animation: slideUp 0.7s ease 0.2s forwards;
}
.hero-actions {
  display: flex; align-items: center; gap: 16px; flex-wrap: wrap;
  opacity: 0; animation: slideUp 0.7s ease 0.3s forwards;
}
.btn-primary {
  display: inline-flex; align-items: center; gap: 10px;
  background: var(--wa-green); color: white;
  padding: 16px 34px; border-radius: 60px; font-size: 16px;
  font-weight: 600; text-decoration: none; border: none; cursor: pointer;
  transition: all 0.3s; box-shadow: 0 4px 24px rgba(37,211,102,0.2);
  font-family: var(--sans);
}
...
.proof-face.f1 { background: var(--forest); }
.proof-face.f2 { background: var(--ember); }
.proof-face.f3 { background: var(--wa-dark); }
.proof-face.f4 { background: #8B6B4E; }
.proof-text { font-size: 13px; color: var(--text-faint); line-height: 1.4; }
.proof-text strong { color: var(--text-soft); font-weight: 600; }
...
.hero-visual {
  ...
  position: relative;
  opacity: 0; animation: slideUp 0.8s ease 0.35s forwards;
}

/* ─── PHONE ─── */
.phone-frame {
  width: 290px; background: #1A1A1A; border-radius: 38px;
  padding: 10px; position: relative; z-index: 2;
  box-shadow: 0 40px 80px rgba(0,0,0,0.12), 0 2px 6px rgba(0,0,0,0.08),
    inset 0 1px 0 rgba(255,255,255,0.04);
}
.phone-notch {
  width: 90px; height: 22px; background: #1A1A1A;
  border-radius: 0 0 14px 14px; margin: 0 auto; margin-top: -2px;
}
.phone-screen {
  background: #ECE5DD; border-radius: 28px; overflow: hidden; min-height: 420px;
  background-image: url("data:image/svg+xml,%3Csvg width='60' height='60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='%23d4cfc6' fill-opacity='0.12'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/svg%3E");
}
.wa-header {
...
.wa-avatar {
  ...
  font-size: 14px; color: var(--forest-deep); font-weight: 700;
}
.wa-name { font-size: 14px; font-weight: 600; }
.wa-status { font-size: 11px; opacity: 0.7; }
.chat-area { padding: 12px 10px; }
.bubble {
  max-width: 82%; padding: 8px 11px; border-radius: 10px; margin-bottom: 5px;
  font-size: 12.5px; line-height: 1.5; position: relative;
}
.bubble.bot {
  background: white; margin-right: auto; border-top-left-radius: 3px;
  box-shadow: 0 1px 1px rgba(0,0,0,0.04);
}
.bubble.user {
  background: #DCF8C6; margin-left: auto; border-top-right-radius: 3px;
}
.bubble .time { font-size: 10px; color: #93918B; text-align: right; margin-top: 3px; }
.voice-msg {
  display: flex; align-items: center; gap: 8px;
  background: white; padding: 8px 11px; border-radius: 10px; margin-bottom: 5px;
  border-top-left-radius: 3px; max-width: 72%;
  box-shadow: 0 1px 1px rgba(0,0,0,0.04);
}
.voice-play {
  width: 28px; height: 28px; border-radius: 50%; background: var(--wa-green);
...
.qr {
  ...
  padding: 4px 9px; background: rgba(20,92,72,0.05); border-radius: 6px;
  font-size: 11.5px; color: var(--forest-deep);
}
.chat-area > * { opacity: 0; animation: msgPop 0.35s ease forwards; }
.chat-area > :nth-child(1) { animation-delay: 0.6s; }
.chat-area > :nth-child(2) { animation-delay: 1s; }
.chat-area > :nth-child(3) { animation-delay: 1.5s; }
.chat-area > :nth-child(4) { animation-delay: 1.9s; }
.chat-area > :nth-child(5) { animation-delay: 2.3s; }
.chat-area > :nth-child(6) { animation-delay: 2.7s; }
@keyframes msgPop {
...
.section-heading {
  ...
  color: var(--forest-deep); margin-bottom: 12px; letter-spacing: -0.8px;
  line-height: 1.2; font-weight: 700;
}
.section-desc {
  font-size: 16px; color: var(--text-soft); max-width: 500px; line-height: 1.7;
  margin-bottom: 48px;
}
.steps-grid {
...
.step-card {
  background: white; border-radius: var(--r-lg); padding: 32px 26px;
  border: 1px solid rgba(20,92,72,0.06);
  position: relative; overflow: hidden;
...
.step-card::before {
  ...
  color: var(--sage); line-height: 1; pointer-events: none;
}
.step-icon {
...
.step-time {
  ...
  background: var(--sage); padding: 4px 12px; border-radius: 50px;
}
...
.experience-section::before {
  ...
  background: radial-gradient(circle, rgba(37,211,102,0.07) 0%, transparent 60%);
}
...
.price-card {
  ...
  border: 1px solid rgba(20,92,72,0.08);
  box-shadow: var(--shadow-lift);
  position: relative; overflow: hidden; text-align: left;
}
.price-gradient-bar {
  position: absolute; top: 0; left: 0; right: 0; height: 4px;
  background: linear-gradient(90deg, var(--wa-green), var(--forest));
}
.price-tag {
...
.price-amount {
  font-family: var(--serif); font-size: 52px; color: var(--forest-deep);
  font-weight: 700; letter-spacing: -2px; line-height: 1;
}
.price-amount .currency { font-size: 26px; vertical-align: super; }
.price-amount .period {
  font-size: 16px; color: var(--text-faint); font-family: var(--sans);
...
.price-cta {
  ...
  background: var(--wa-green); color: white;
  padding: 15px; border-radius: 60px; font-size: 16px;
  font-weight: 600; border: none; cursor: pointer;
...
/* ─── FOOTER ─── */
footer {
  background: var(--forest-deep); color: rgba(255,255,255,0.5);
  padding: 0 0 36px; font-size: 13px;
...
.modal-overlay {
  ...
  background: rgba(11,61,48,0.5); backdrop-filter: blur(8px);
...
.modal-close {
  ...
  display: flex; align-items: center; justify-content: center;
  color: var(--text-soft); font-size: 20px; transition: background 0.2s;
  font-family: var(--sans);
}
.modal-close:hover { background: var(--sage-mid); }
.modal h2 {
  font-family: var(--serif); font-size: 26px; color: var(--forest-deep);
  font-weight: 700; letter-spacing: -0.5px;
}
.modal-sub { font-size: 14px; color: var(--text-soft); margin: 4px 0 24px; line-height: 1.5; }
...
.form-submit {
  ...
  background: var(--wa-green); color: white;
  border: none; border-radius: 60px; font-size: 15px; font-weight: 600;
...
.form-back {
  ...
  border: none; border-radius: 60px; font-size: 14px; font-weight: 600;
  cursor: pointer; font-family: var(--sans);
